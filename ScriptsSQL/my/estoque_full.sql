...
create table compras(
	...
	constraint fornecedor_fk foreign key (fornecedor) references fornecedores(id)
);
--
create table compra_itens( -- alimentada com dados de produtos quando cadastrando os pedidos
	id int primary key auto_increment,
	compra int not null,
...
-- 
create table estoque(-- Cada compra e cada venda de produto atualizar o estoque. Na compra a atualização é manual e na venda automático
	id int primary key auto_increment, 
	...
	constraint armazem_est_fk foreign key (armazem) references armazens(id),
	constraint compra_est_fk foreign key (compra_item) references compra_itens(id)
);
--
...
--  data_confirmacao - data da baixa no estoque. Na compra alertar sobre o estoque_min
create table bonus(
	id int primary key auto_increment,
...
create table comissoes(
	...
	data_pedido datetime not null,
	pedido int not null,
	constraint pedido_com_fk foreign key(pedido) REFERENCES pedidos (id)
);
-- valor_comissao -  -- em R$, 5% sobre o total do pedido
...
create table pedido_itens(
	...
	pedido int not null,
	constraint estoque_ped_fk foreign key (id) references estoque(id),
	constraint pedido_ped_fk foreign key (pedido) references pedidos(id)
);
...
ALTER TABLE clientes ADD CHECK (data_nasc >'1900-01-01' AND data_nasc < CURRENT_TIMESTAMP);
ALTER TABLE clientes ADD CHECK (POSITION('@' IN email)>0);
--
ALTER TABLE estoque ADD CHECK (quant_estoque > 0);
ALTER TABLE `estoque` MODIFY COLUMN `data_atualizacao` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP;
--
--
